...
export function WhoForSection() {
  return (
    <section className="section-muted border-y border-border py-20 md:py-28 lg:py-32">
        ...
        <div className="grid gap-5 sm:grid-cols-2 md:gap-6">
          {audiences.map((audience, i) => (
            <RevealOnScroll key={audience.label} delayMs={i * 75}>
              <div
                className={cn(
                  "group h-full rounded-xl border border-border bg-card p-7 text-card-foreground shadow-sm shadow-black/20 md:p-8",
                  "transition-[border-color,box-shadow] duration-300 ease-out",
                  "hover:border-primary/40 hover:shadow-lg hover:shadow-black/25"
                )}
              >
                {/* Icon + label */}
