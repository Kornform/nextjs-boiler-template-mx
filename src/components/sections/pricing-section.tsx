...
export function PricingSection() {
  return (
    <section
      id="pricing"
      className="scroll-mt-24 border-y border-border bg-muted/20 py-20 md:py-28 lg:py-32"
    >
        ...
        <div className="content-pricing grid gap-6 text-left md:grid-cols-2 md:gap-8">
          {tiers.map((tier, i) => (
            <RevealOnScroll key={tier.name} delayMs={i * 90}>
              <Card
                className={cn(
                  "relative flex flex-col overflow-hidden rounded-2xl border-2 bg-card p-0 shadow-sm shadow-black/20",
                  "transition-[transform,box-shadow,border-color] duration-300",
                  tier.highlighted
                    ? "z-[1] border-primary shadow-lg shadow-primary/20 md:scale-[1.02]"
                    : "border-border hover:border-muted-foreground/40"
                )}
              >
                {tier.highlighted && (
                  <div className="absolute right-0 top-0 rounded-bl-lg rounded-tr-xl bg-primary px-3.5 py-1.5 font-display text-[0.65rem] font-bold tracking-wider text-primary-foreground uppercase">
                    Most Popular
                  </div>
                )}
                <div
